...
start_link(Port, Module, ListenerName, ConfigItem, LoopFunc) when is_integer(Port), is_atom(Module) ->
    ...
    gen_server:start_link({local, ListenerName}, ?MODULE, State, []). 
    

init(State = #server_state{port=Port, configitem=ConfigItem}) ->
    Opts = mijkutils:get_obelisk_socket_options(obelisk, ConfigItem),
