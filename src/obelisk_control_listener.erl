-module(obelisk_control_listener).
...
handle_info({inet_async, ListSock, Ref, {ok, CliSocket}},
            #state{listener=ListSock, acceptor=Ref,
                                    module=Module, socket_o=Options} = State) ->
    try
        case set_sockopt(ListSock, CliSocket, Options) of
        ok              -> ok;
        ...
        %% supervisor.
        logger:log({debug, "listener: new client connected"}),
        {ok, Pid} = obelisk_app:start_client(),
%gen_tcp:controlling_process(CliSocket, Pid),
        mijktcp:controlling_process(CliSocket, Pid, Options),
